...
const makeProps = (
  overrides: Partial<SwatchCardProps> = {},
): SwatchCardProps => ({
  id: "testColor",
  title: "Links",
  color: red,
...
describe("SwatchCard — source swatch", () => {
  ...
  it("keeps the source swatch stable when color and sourceColor differ", () => {
    ...
    expect(
      within(screen.getByRole("group", { name: "Source" })).getByText(
        "rgb(255, 0, 0)",
      ),
    ).toBeInTheDocument();
    expect(
      within(screen.getByRole("group", { name: "Compare" })).getByText(
        "rgb(0, 0, 255)",
      ),
    ).toBeInTheDocument();
  });
...
describe("SwatchCard — slider interaction", () => {
  it("calls onSetColor when the saturation slider changes", () => {
    ...
    fireEvent.change(document.getElementById("testColorSat")!, {
      target: { value: "50" },
    });
  ...
  it("calls onSetColor when the lightness slider changes", () => {
    ...
    fireEvent.change(document.getElementById("testColorLight")!, {
      target: { value: "30" },
    });
