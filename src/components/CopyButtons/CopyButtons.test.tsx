...
describe("CopyButtons — clipboard supported", () => {
  ...
  beforeEach(() => {
    mockCopy = vi.fn();
    vi.mocked(useClipboard).mockReturnValue({
      copy: mockCopy as (text: string, key: string) => void,
      copiedKey: null,
      isSupported: true,
    });
  });
  ...
  it("each button has a descriptive title attribute", () => {
    render(<CopyButtons hex={hex} rgb={rgb} hsl={hsl} />);
    expect(screen.getByRole("button", { name: "HEX" })).toHaveAttribute(
      "title",
      "Copy #ff0000",
    );
    expect(screen.getByRole("button", { name: "RGB" })).toHaveAttribute(
      "title",
      "Copy rgb(255, 0, 0)",
    );
    expect(screen.getByRole("button", { name: "HSL" })).toHaveAttribute(
      "title",
      "Copy hsl(0, 100%, 50%)",
    );
  });
