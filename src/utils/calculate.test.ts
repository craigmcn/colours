...
describe("calculateColorArray", () => {
  it("returns background at opacity 0", () => {
    expect(calculateColorArray(0, [255, 255, 255], [0, 0, 0])).toEqual([
      255, 255, 255,
    ]);
  });

  it("returns foreground at opacity 1", () => {
    expect(calculateColorArray(1, [255, 255, 255], [0, 91, 153])).toEqual([
      0, 91, 153,
    ]);
  });
  ...
  it("blends at 50% opacity", () => {
    ...
    expect(calculateColorArray(0.5, [0, 0, 0], [255, 255, 255])).toEqual([
      128, 128, 128,
    ]);
  });
  ...
  it("blends at 25% opacity — default scenario", () => {
    ...
    expect(calculateColorArray(0.25, [255, 255, 255], [0, 91, 153])).toEqual([
      191, 214, 230,
    ]);
  });
...
describe("calculateFg", () => {
  ...
  it("handles NaN gracefully — returns 0 per channel", () => {
    const result = calculateFg(
      0.5,
      [255, 255, 255],
      [NaN as unknown as number, 100, 100],
    );
