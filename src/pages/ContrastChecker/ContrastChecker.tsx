...
export const ContrastChecker = () => {
  ...
  const link2Body = useMemo(
    () => contrastRatio(link.color.rgb, text.color.rgb),
    [link.color.rgb, text.color.rgb],
  );
  const link2Bg = useMemo(
    () => contrastRatio(link.color.rgb, bg.color.rgb),
    [link.color.rgb, bg.color.rgb],
  );
  const body2Bg = useMemo(
    () => contrastRatio(text.color.rgb, bg.color.rgb),
    [text.color.rgb, bg.color.rgb],
  );
  ...
  const handleCalculateWcag = (level: "AA" | "AAA") => {
    ...
    text.set(
      { hex: textHex, rgb: textRgb, hsl: textHsl },
      hex2Str(textHex),
      true,
    );
    ...
    const {
      ...
      rgb: linkRgb,
      hsl: linkHsl,
    } = getContrastColour({ color: textRgb, wcag: "A", direction });
    link.set(
      { hex: linkHex, rgb: linkRgb, hsl: linkHsl },
      hex2Str(linkHex),
      true,
    );
  };
    ...
    <main className="main">
      <section className="flex flex--grid">
        <SwatchCard
          ...
          inputValue={link.inputValue}
          onInputChange={link.update}
          onSetColor={link.set}
        />
        <SwatchCard
          id="textColor"
          title="Text"
          color={text.color}
          sourceColor={text.sourceColor}
          inputValue={text.inputValue}
          onInputChange={text.update}
          onSetColor={text.set}
        />
        <SwatchCard
          id="bgColor"
          title="Background"
          color={bg.color}
          sourceColor={bg.sourceColor}
          inputValue={bg.inputValue}
          onInputChange={bg.update}
          onSetColor={bg.set}
        />
      </section>
      ...
      <div className="flex flex--grid">
        <div className="flex__item flex__item--12 flex__item--9-lg m-v-md">
          <section>
            <ContrastResult
              type="link2Body"
              label="Link to Body text"
              ratio={link2Body}
              results={getPassFail(link2Body, "link2Body")}
            />
          </section>
          <section className="m-v-md">
            <ContrastResult
              ...
              ratio={link2Bg}
              results={getPassFail(link2Bg, "link2Bg")}
            />
          </section>
          <section>
            <ContrastResult
              type="body2Bg"
              label="Body text to Background"
              ratio={body2Bg}
              results={getPassFail(body2Bg, "body2Bg")}
            />
          </section>
        </div>
        ...
        <div className="card flex__item flex__item--12 flex__item--3-lg m-v-md">
          <div className="card__body">
            ...
            <button
              ...
              onClick={() => handleCalculateWcag("AA")}
            >
              WCAG AA
            </button>
            <button
              ...
              type="button"
              style={{ display: "block", width: "100%" }}
              onClick={() => handleCalculateWcag("AAA")}
            >
              WCAG AAA
            </button>
          </div>
