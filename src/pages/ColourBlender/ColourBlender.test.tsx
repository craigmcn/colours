...
const setSteps = (n: number) =>
  fireEvent.change(stepsInput(), { target: { value: String(n) } });
const hexButtonCount = () =>
  screen.getAllByRole("button", { name: "HEX" }).length;
...
describe("default blend swatches", () => {
  it("first swatch is the start colour", () => {
    renderComponent();
    expect(
      within(swatchGroups()[0]).getByText("rgb(0, 91, 153)"),
    ).toBeInTheDocument();
  ...
  it("last swatch is the end colour", () => {
    ...
    expect(
      within(groups[groups.length - 1]).getByText("rgb(255, 255, 255)"),
    ).toBeInTheDocument();
    expect(
      within(groups[groups.length - 1]).getByText("#ffffff"),
    ).toBeInTheDocument();
  });
  ...
  it("first swatch includes hsl value", () => {
    renderComponent();
    expect(
      within(swatchGroups()[0]).getByText("hsl(204, 100%, 30%)"),
    ).toBeInTheDocument();
  });
});
...
describe("blend correctness", () => {
  ...
  it("swatches lighten from start to end when blending dark to light", async () => {
    ...
    setSteps(2);
    const groups = swatchGroups();
    const idx0 = groups.findIndex((g) =>
      g.textContent!.includes("rgb(0, 0, 0)"),
    );
    const idx1 = groups.findIndex((g) =>
      g.textContent!.includes("rgb(128, 128, 128)"),
    );
    const idx2 = groups.findIndex((g) =>
      g.textContent!.includes("rgb(255, 255, 255)"),
    );
...
describe("copy interactions", () => {
  it("clicking HEX calls copy with a valid hex string", async () => {
    ...
    await user.click(
      within(swatchGroups()[0]).getByRole("button", { name: "HEX" }),
    );
  ...
  it("clicking RGB calls copy with an rgb() string", async () => {
    ...
    await user.click(
      within(swatchGroups()[0]).getByRole("button", { name: "RGB" }),
    );
  ...
  it("clicking HSL calls copy with an hsl() string", async () => {
    ...
    await user.click(
      within(swatchGroups()[0]).getByRole("button", { name: "HSL" }),
    );
  ...
  it("first swatch HEX button copies the start colour", async () => {
    ...
    await user.click(
      within(screen.getByRole("group", { name: "#005b99" })).getByRole(
        "button",
        { name: "HEX" },
      ),
    );
  ...
  it("last swatch HEX button copies the end colour", async () => {
    ...
    await user.click(
      within(screen.getByRole("group", { name: "#ffffff" })).getByRole(
        "button",
        { name: "HEX" },
      ),
    );
...
describe("swatch layout", () => {
  ...
  it("swatches are ordered start → end (dark to light for default colours)", () => {
    ...
    const groups = swatchGroups();
    const idx0 = groups.findIndex((g) =>
      g.textContent!.includes("rgb(0, 91, 153)"),
    );
    const idx1 = groups.findIndex((g) =>
      g.textContent!.includes("rgb(51, 124, 173)"),
    );
    const idx2 = groups.findIndex((g) =>
      g.textContent!.includes("rgb(255, 255, 255)"),
    );
