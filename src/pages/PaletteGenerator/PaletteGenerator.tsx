...
const buildPalette = (base: RGB, light: RGB, dark: RGB): RGB[] => {
  const result: RGB[] = [];
  for (let i = 0; i < 5; i++)
    result.push(calculateColorArray(i / 5, light, base));
  for (let i = 0; i < 6; i++)
    result.push(calculateColorArray(i / 5, base, dark));
...
export const PaletteGenerator = () => {
  ...
  const toCssIdent = (s: string): string =>
    s
      .toLowerCase()
      .replace(/\s+/g, "-")
      .replace(/[^a-z0-9-]/g, "") || "colour";

  const colorName = toCssIdent(
      ...
      <div className="flex flex--grid">
        <div className="flex__item flex__item--12 flex__item--3-sm">
          <div className="form__group">
            <label className="form__label" htmlFor="paletteName">
              Name
            </label>
            <input
              id="paletteName"
          ...
          <ColorInput
            id="paletteLight"
            label="Light"
            value={light.inputValue}
            onChange={light.update}
          />
          <ColorInput
            id="paletteDark"
            label="Dark"
            value={dark.inputValue}
            onChange={dark.update}
          />
        </div>
