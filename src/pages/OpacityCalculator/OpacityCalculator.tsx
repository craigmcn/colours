...
export const OpacityCalculator = () => {
  ...
  const [resultRgb, setResultRgb] = useState<RGB>(() =>
    calculateColorArray(
      DEFAULT_OPACITY / 100,
      hex2Rgb(splitHex(DEFAULT_BG)),
      hex2Rgb(splitHex(DEFAULT_FG)),
    ),
  );
  ...
  const displayResultRgb: RGB =
    solveFor === "result" ? solvedResult : resultRgb;
  const displayOpacity: number =
    solveFor === "opacity" ? solvedOpacity : opacity;

  const fgInputValue =
    solveFor === "fg" ? hex2Str(rgb2Hex(solvedFg, false)) : fg.inputValue;
  const bgInputValue =
    solveFor === "bg" ? hex2Str(rgb2Hex(solvedBg, false)) : bg.inputValue;
  const resInputValue =
    solveFor === "result"
      ? hex2Str(rgb2Hex(solvedResult, false))
      : resultInputValue;

  // ─── Edge-case warnings ───────────────────────────────────────────────────────

  const fgWarning =
    solveFor === "fg" && opacity === 0
      ? "Opacity is 0% — foreground has no effect on the result"
      : undefined;
  const bgWarning =
    solveFor === "bg" && opacity === 100
  ...
  return (
    <main className="main main--fixed">
      ...
      <div className={`colours flex ${styles.swatches}`}>
        <div
          className={`colour ${styles.swatch}`}
          style={swatchStyle(displayFgRgb)}
        >
          <strong>Foreground</strong>
          <br />
          <span>{hex2Str(rgb2Hex(displayFgRgb, false))}</span>
          <br />
          <span>{rgb2Str(displayFgRgb)}</span>
          <br />
          <span>{hsl2Str(rgb2Hsl(displayFgRgb))}</span>
        </div>
        <div
          className={`colour ${styles.swatch}`}
          style={swatchStyle(displayResultRgb)}
        >
          <strong>Result</strong>
          <br />
          <span>{hex2Str(resHex)}</span>
          <br />
          <span>{rgb2Str(displayResultRgb)}</span>
          <br />
          <span>{hsl2Str(resHsl)}</span>
        </div>
        <div
          className={`colour ${styles.swatch}`}
          style={swatchStyle(displayBgRgb)}
        >
          <strong>Background</strong>
          <br />
          <span>{hex2Str(rgb2Hex(displayBgRgb, false))}</span>
          <br />
          <span>{rgb2Str(displayBgRgb)}</span>
          <br />
      ...
      <div className="form__group">
        <label className="form__label" htmlFor="opacity">
          Opacity
        </label>
        <input
          id="opacity"
        ...
        <p className="text--muted">
          <code>
            opacity: <span>{useDecimal(displayOpacity / 100)}</span> (
            <span>{displayOpacity.toFixed(0)}%</span>)
          </code>
        </p>
      </div>
    </main>
