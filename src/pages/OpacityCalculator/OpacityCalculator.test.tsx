...
describe("initial render", () => {
  it("renders the page heading", () => {
    renderComponent();
    expect(
      screen.getByRole("heading", { name: /calculate opacity/i }),
    ).toBeInTheDocument();
  });
  ...
  it("renders solve-for radio options for all four fields", () => {
    renderComponent();
    expect(
      screen.getByRole("radio", { name: "Foreground" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("radio", { name: "Background" }),
    ).toBeInTheDocument();
...
describe("solve for Opacity", () => {
  ...
  it("calculates a high opacity when result is close to foreground", async () => {
    ...
    expect(
      parseInt((opSlider() as HTMLInputElement).value),
...
describe("colour swatches", () => {
  ...
  it("background swatch shows the default background colour values", () => {
    ...
    expect(
      within(bgSwatch).getByText("rgb(255, 255, 255)"),
    ).toBeInTheDocument();
  });
  ...
  it("result swatch shows the default computed result", () => {
    ...
    expect(
      within(resSwatch).getByText("rgb(191, 214, 230)"),
    ).toBeInTheDocument();
  });
